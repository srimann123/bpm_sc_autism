...
# ------------------------------------------------------------
# These functions set/reset state that can affect run-to-run
# reproducibility. They are used before the major clustering
# stages so downstream stochastic steps start from the same seed.
# ============================================================
...
# ============================================================
# Condition/config parsing
# ------------------------------------------------------------
# The CSV row for a run is converted into condition_params, which
# is the main settings dictionary passed through the entire pipeline.
# This section standardizes semicolon-separated values, scalar values,
# and per-field counts so later functions can assume a consistent shape.
# ============================================================
...
# ============================================================
# AnnData graph/embedding cleanup
# ------------------------------------------------------------
# After outlier removal, the previous PCA, neighbor graph, clusters,
# and embeddings no longer match the filtered cell set. This helper
# removes those stale fields before recomputing them.
# ============================================================
...
# ------------------------------------------------------------
# This stage moves from the expression matrix to a PCA representation
# and then to a k-nearest-neighbor graph. That graph is the foundation
# for clustering and UMAP, so the diagnostic prints check graph quality
# before the rest of the analysis continues.
# ============================================================
...
# Clustering stage
# ------------------------------------------------------------
# Leiden and Louvain are run on the neighbor graph created above.
# The cluster assignments are written out so nuclei can be traced back
# to their assigned cluster before or after outlier removal.
# ============================================================
...
# Embedding stage
# ------------------------------------------------------------
# UMAP and t-SNE are generated after clustering for visualization.
# PCA/neighbors are produced with RAPIDS, but embeddings are run through
# Scanpy on CPU here to keep the visualization step more reproducible.
# ============================================================
...
# ============================================================
# HVG quality-control summaries
# ------------------------------------------------------------
# This optional QC section summarizes variation across selected HVGs
# before downstream scaling. It is useful for checking whether selected
# genes have near-zero variance or unusual sparsity patterns.
# ============================================================
...
# ============================================================
# Post-clustering QC summaries
# ------------------------------------------------------------
# These summaries describe each final cluster after filtering, including
# cluster size and simple expression-distribution metrics. They provide
# a compact way to evaluate what changed after outlier removal.
# ============================================================
...
# ------------------------------------------------------------
# Plotting functions are grouped here so figure formatting, legend space,
# and output naming stay consistent across UMAP and t-SNE plots.
# ============================================================
...
# ============================================================
# Main before/after analysis workflow
# ------------------------------------------------------------
# This is the central analysis routine. It runs the full clustering and
# visualization workflow once on all nuclei, detects outlier nuclei, drops
# them, clears stale graph state, and then reruns the same workflow on the
# filtered data. Outputs from both passes are saved under the same run folder.
# ============================================================
...
# ------------------------------------------------------------
# This section mirrors the original R-style outlier workflow. It measures
# how much each nucleus deviates from its cluster expression profile, uses
# MAD-based thresholds to flag outliers, and returns both per-sample summary
# tables and the exact nuclei to remove.
# ============================================================
...
# ============================================================
# Covariate encoding and residualization setup
# ------------------------------------------------------------
# These functions prepare covariates for regression-style correction.
# Categorical covariates are dummy-encoded, moved to GPU, and converted
# into an orthonormal Q matrix so expression batches can be residualized
# efficiently with projection math.
# ============================================================
...
# ============================================================
# Run-label and metadata helpers
# ------------------------------------------------------------
# These helpers create stable labels and hashes for each run based on
# input files, covariates, and major analysis parameters. The goal is to
# make output folders traceable and reduce accidental overwriting.
# ============================================================
...
# ------------------------------------------------------------
# This section implements a CellRanger-style HVG selection workflow.
# Genes are binned by mean expression, dispersion is normalized within
# bins, optional annotation filters are applied, and the top variable
# genes are selected for downstream PCA/clustering.
# ============================================================
...
# ============================================================
# Input file alignment
# ------------------------------------------------------------
# Before combining datasets, the pipeline finds genes shared across all
# input H5 files and collects nuclei IDs in file order. This keeps the
# expression matrix, covariates, and AnnData labels aligned.
# ============================================================
...
# ------------------------------------------------------------
# These functions normalize, optionally residualize, scale, clip, and
# filter expression batches. They operate on sparse matrices where possible
# so large single-cell matrices can be processed in manageable chunks.
# ============================================================
...
# ============================================================
# Batch-level gene processing and HVG statistics
# ------------------------------------------------------------
# This is the first large matrix-building pass. Shared genes are processed
# in batches across all datasets, low-expression genes are removed, batches
# are normalized/residualized, and per-gene statistics are collected for
# HVG selection.
# ============================================================
...
# ============================================================
# Final HVG matrix construction
# ------------------------------------------------------------
# After HVGs are selected, this section rebuilds the final expression matrix
# using only those genes, applies the same normalization/residualization
# pipeline, and returns the matrix that will become the AnnData object.
# ============================================================
